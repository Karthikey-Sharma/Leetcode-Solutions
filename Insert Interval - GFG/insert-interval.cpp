  ...
  public:
    vector<vector<int>> insertNewEvent(int N, vector<vector<int>> &intervals, vector<int> &newInterval) {
        ...
        while(i < intervals.size()){
            if(intervals[i][1] < newInterval[0]){ /// agar mera interval ka end point < new interval ka starting point
                result.push_back(intervals[i]);
            }else if(intervals[i][0] > newInterval[1]){/// agar mere interval ka starting point > new interval ke end point 
                break;
            }else{
